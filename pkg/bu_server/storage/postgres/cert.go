...
func (s *_Storage) AddRootCert(ctx context.Context, tx storage.Tx, ts int64, fingerPrint string, cert []byte) error {
	query := `
...
func (s *_Storage) RevokeRootCert(ctx context.Context, tx storage.Tx, ts int64, fingerPrinter string) error {
	query := `
UPDATE root_cert
...
func (s *_Storage) GetActiveRootCert(ctx context.Context, tx storage.Tx) ([][]byte, error) {
	query := `
SELECT cert
FROM root_cert
WHERE revoked = false
...
func (s *_Storage) AddCRL(ctx context.Context, tx storage.Tx, ts int64, issuerKeyID string, certSerialNumber string, revokedAt int64, crl []byte) error {
	query := `
...
func (s *_Storage) GetCRL(ctx context.Context, tx storage.Tx, req storage.GetCRLRequest) (storage.GetCRLResult, error) {
	query := `
