<script lang="ts">
  ...
  const handleSubmit = async () => {
    try {
      ...
      await useUserStore().resetPassword(id, request.password)
      ...
      if (err instanceof yup.ValidationError) {
        ...
        errorMessage = `An error occurred during reset password. Please try again later. (${(err as Error).message})`
      }
    }
  }
    ...
    <div class="modal-content">
      <form class="form-container">
        ...
        <div class="form-field">
          ...
          <input id="confirm" name="confirm" type="password" bind:value={request.confirm} />
          {#if errors.password}<p class="error">{errors.confirm}</p>{/if}
        </div>
        {#if errorMessage}
